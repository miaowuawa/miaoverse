...
- 修改 `scripts/db.sql`（保持完整建库脚本为最新）。
- 在 `service/DBMigration/migrations/` 添加新的自增迁移文件（如 `0002_add_xxx.sql`），迁移文件使用纯 SQL，不要修改已提交的旧迁移文件。
- 不要只改 `db.sql` 而不添加迁移，也不要只添加迁移而不改 `db.sql`；两者缺一都会被判定为数据库变更不完整。
